      ...
      child: Column(
        children: [
          TextFormField(
            ...
            onChanged: (value) {
              if (value.isNotEmpty && errors.contains(nEmailNullError)) {
                setState(() {
                  errors.remove(nEmailNullError);
                });
              } else if (emailValidatorRegExp.hasMatch(value) &&
                  errors.contains(nInvalidEmailError)) {
                setState(() {
                  errors.remove(nInvalidEmailError);
                });
              }
              return;
            },
            validator: (value) {
              if (value!.isEmpty && !errors.contains(nEmailNullError)) {
                setState(() {
                  errors.add(nEmailNullError);
                });
              } else if (!emailValidatorRegExp.hasMatch(value) &&
                  !errors.contains(nInvalidEmailError)) {
                setState(() {
                  errors.add(nInvalidEmailError);
                });
              }
              return null;
            },
